Crafty.c("TiledLevel", {
	...
	makeLayer: function(layer) {
	  ...
	  var iso = Crafty.isometric.size(lWidth*2);
	  var tileNr = 0;
	  for (x = 0; x < lWidth; x++){
		for (y = 0; y < lHeight; y++){
			//console.log("tile "+tileNr+" x: "+x+" y: "+y);
			iso.place(x, y, 0, Crafty.e("tile" + lData[tileNr]));
			tileNr++;
		}
	  }
	  ...
	  // for (i = 0, _len = lData.length; i < _len; i++) {
		// tDatum = lData[i];
		// if (tDatum) {
		  // var tile = Crafty.e("2D, DOM, tile" + tDatum);
		  // iso.place((i % lWidth) * tile.w, (i / lWidth | 0) * tile.h, 0, tile);
		// }
	  // }
	  return null;
	},
